...
app.post('/api/fetch-json', async (req, res) => {
    const {url} = req.body;
    try {
        const alexResponse = await axios.get(url);
        const alexResults = alexResponse.data.results;
        const filteredAlexResults = alexResults.map(item =>({
            doi:item.doi,
            title:item.title,
...
//         const generatedUrl = "https://api.openalex.org/works?filter=publication_year:2020";
...
//         const articles = extractedResults.map((article) => ({
//             doi: article.doi,
//             title: article.title,
//             publication_year: article.publication_year,
//             is_oa: article.primary_location ? article.primary_location.is_oa : false,
